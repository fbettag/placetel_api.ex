...
defmodule PlacetelAPI.Connection do
  @moduledoc """
  ...
  - `options`: a keyword list where to override the :api_key per connection instead of using the global configured one.
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:placetel_api, :base_url, "https://api.placetel.de/v2")
      )

    api_key = Keyword.get(options, :api_key, Application.get_env(:placetel_api, :api_key))
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "PlacetelAPI 2.0.0 Tesla/Elixir"
        )
      )
